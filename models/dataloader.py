...
class DflDataModule(L.LightningDataModule):
    def __init__(self, batch_size, num_workers, seed) -> None:
        super().__init__()
        self.batch_size = batch_size
        self.num_workers = num_workers
        train_data = open('data/DFL/TRAINING.fasta').read().splitlines()
        test_data = open('data/DFL/TEST.fasta').read().splitlines()
        ...
        # shuffle train set with seed
        indices = list(range(len(self.train_set)))
        random.Random(seed).shuffle(indices)
        self.train_idx, self.val_idx = indices[:int(len(indices) * 0.8)], indices[int(len(indices) * 0.8):]

    def train_dataloader(self):
        return DataLoader([self.train_set[i] for i in self.train_idx],
                          batch_size=self.batch_size, num_workers=self.num_workers, shuffle=True,
                          collate_fn=self.collate)

    def val_dataloader(self):
        return DataLoader([self.train_set[i] for i in self.val_idx],
