...
from dataloader import (ApodDataModule, Caid2DataModule, CaidDataModule,
                        DeepDisoBindDataModule, DflDataModule,
                        DisoLipPredDataModule, DisProt2022DecDataModule,
                        Dm4229DataModule, NetSurfP30DataModule,
    ...
    for model_name in tqdm([
        ...
        match task:
            case 'idp':
                ...
                dm4229 = Dm4229DataModule(batch_size=1, num_workers=1)
                tests = VariousIdpTestDataModule(batch_size=1, num_workers=1)
                disprot = DisProt2022DecDataModule(batch_size=1, num_workers=1)
                t = itertools.chain(
                    # fldpnn.train_dataloader(),
                    # fldpnn.val_dataloader(),
                    # fldpnn.test_dataloader(),
                    # caid.test_dataloader(),
                    # dm4229.train_dataloader(),
                    # dm4229.val_dataloader(),
                    # tests.test_dataloader('casp'),
                    # tests.test_dataloader('disorder723'),
                    # disprot.test_dataloader(),
                    caid2.test_dataloader(),
                    #    tests.test_dataloader('mxd494'),
                    #     tests.test_dataloader('sl329'),
                )
            case 'linker':
                # dm = DflDataModule(batch_size=1, num_workers=24, seed=42, n_splits=4)
            ...
            case 'lipid':
                dm = DisoLipPredDataModule(batch_size=1, num_workers=1)
